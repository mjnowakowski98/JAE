...
class AnimCommandHandler {
    constructor(_objMan) {
        ...
        this.getObjectManager = () => objectManager;
        ...
        this.executeCommand = function(commandDescriptor) {
            ...
            return command.execute(objMan); // Execute and return engine response
        }

        this.undoLastCommand = function() {
            let tmpCommand = previousCommands.pop(); // Pop last executed command
            return tmpCommand.undo(objMan); // Undo and return engine response
        }
    }
}
